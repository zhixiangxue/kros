...
class BrowseDriver(Protocol):
    ...
    Return-type principle: any op that could change what's visible on
    the page returns a full :class:`ReadResult` — same mental model as
    a human interacting with a browser, who naturally *sees* the page
    after every action without a separate "look" step. Ops that don't
    mutate the view (``find`` / ``eval`` / ``wait``) keep their focused
    return shape.
    """
