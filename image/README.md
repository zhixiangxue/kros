...
- [ ] `kros memory` —— 封装 [seeka-ai](https://github.com/zhixiangxue/seeka-ai)
- [ ] `kros sandbox` —— 封装 [doka-ai](https://github.com/zhixiangxue/doka-ai)
- [ ] `kros browse` —— 底层待评估（lightpanda / obscura / ...）
- [ ] `kros -h` 返回 skill.md
...
新增子命令的约定：在 `commands/` 下新建一个文件，提供 `register(app: typer.Typer)`，然后在 `cli.py` 里 import + 调用一次 `register`。
